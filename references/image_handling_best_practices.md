...
4. **Native draw.io Diagrams** - Preserve editable `.drawio` source attachments with the draw.io macro
5. **External Images** - Referenced via URL (not uploaded as attachments)
...
**Use Case:** Editable network/component diagrams maintained by the Confluence draw.io plugin.

**Workflow:**
1. Keep the editable source as a `.drawio` attachment with content type `application/vnd.jgraph.mxfile`.
2. Use a native `ac:structured-macro ac:name="drawio"` storage macro with a matching `diagramName`.
3. For localized replacements, use `replace_macro_with_drawio.py` in dry-run mode first.
4. Apply only after preview review and explicit confirmation.
5. Read the page back and verify the draw.io macro and matching attachment remain present.

**Important:** The PNG preview generated by draw.io is macro-managed output, not a replacement for the editable `.drawio` source.
